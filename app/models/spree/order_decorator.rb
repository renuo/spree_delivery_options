Spree::Order.class_eval do
  ...
  def valid_delivery_date?
    ...
    if self.delivery_date
      ...
      cutoff_time = Time.now.change(hour: Spree::Config.delivery_cut_off_hour)
      if self.delivery_date == Date.tomorrow && Time.now > cutoff_time
        self.errors[:delivery_date] << "cannot be tomorrow if the order is created after #{Spree::Config.delivery_cut_off_hour}"
      end
    end
